...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'games_game_id_len' and conrelid = 'public.games'::regclass) then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'elo_game_id_len' and conrelid = 'public.elo'::regclass) then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'features_game_id_len' and conrelid = 'public.features'::regclass) then
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'predictions_game_id_len' and conrelid = 'public.predictions'::regclass) then
